...
########  
  ...
  for (j in 1:2){
    tPsi[j, 1:n.regions] ~ dmnorm(ceros[], (D - alpha[j] * W))
    alpha[j] ~ dunif(0.1, 0.9)
  }
  ...
  for (i in 1:2){
   ...
   sigmasq_u1 ~ dgamma (2, 0.05)
   sigmasq_u2 ~ dgamma (2, 0.1)
   rho_u ~ dunif(0.1, 1)
   
   Sigma[1,1] <- sigmasq_u1
   Sigma[2,2] <- sigmasq_u2
   Sigma[1,2] <- sqrt(sigmasq_u1)*sqrt(sigmasq_u2)*rho_u
   Sigma[2,1] <- Sigma[1,2]
